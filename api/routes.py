...
_WRITE_PERMISSION_RULES: tuple[tuple[str, tuple[str, ...], str], ...] = (
    ...
    # Phase 3: Bot 配置管理
    ("POST", ("/bot-configs",), "bot:manage"),
    ("PUT", ("/bot-configs/",), "bot:manage"),
    ("DELETE", ("/bot-configs/",), "bot:manage"),
)
...
def _is_public_path(path: str) -> bool:
    return path in _JWT_EXEMPT_PATHS or path.startswith("/mock-api/") or path in {"/traffic/ingest", "/traffic/batch-ingest"} or path == "/traffic/proxy-config" or path.startswith(_BOT_WEBHOOK_PREFIX)
...
async def api_key_middleware(request: Request, call_next):
    ...
    if request.headers.get("upgrade", "").lower() == "websocket":
        return await call_next(request)
    # 公开路径跳过
    if request.url.path in _EXEMPT_PATHS or request.url.path.startswith("/mock-api/") or request.url.path in {"/traffic/ingest", "/traffic/batch-ingest", "/traffic/proxy-config"} or request.url.path.startswith(_BOT_WEBHOOK_PREFIX):
        return await call_next(request)
...
# Phase 3: Bot 集成
from api.routers.bot_configs import router as bot_configs_router
from api.routers.bot_webhook import router as bot_webhook_router
...
app.include_router(bot_configs_router)  # Phase 3: Bot 配置管理
app.include_router(bot_webhook_router)  # Phase 3: Bot Webhook 接收
